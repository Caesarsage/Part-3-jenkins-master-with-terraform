...
module "alb_jenkins_sg" {
  source = "./modules/security-group_module/jenkins_alb_security_module"
  vpc_id = module.vpc_module.vpc_id
}
...
module "jenkins_master_sg" {
  ...
  bastion_host_sg_id = module.bastion_host_sg.bastion_host_sg_id
}
...
output "jenkins-master-alb" {
  value = module.jenkins_master.jenkins-master-alb
}